...
import type { ProposalCategory, ProposalDetail, CreateProposalRequest } from '@mmf/shared'
import { DatePickerInput } from '../components/ui/DatePickerInput'

export interface ProposalFormPageProps {
  proposalId?: string
}
...
function buildPayload(state: FormState): CreateProposalRequest {
  return {
    title: state.title,
    category: state.category as ProposalCategory,
...
export function ProposalFormPage({ proposalId }: ProposalFormPageProps) {
  ...
  // Populate form from existing proposal data (edit mode)
  useEffect(() => {
    if (existingProposal && !initializedRef.current) {
      initializedRef.current = true
      dispatch({ type: 'INIT', data: existingProposal })
    }
  }, [existingProposal])
  ...
  const {
    ...
    onSuccess: (result) => {
      if (!proposalId) {
        void navigate(`/proposals/${result.id}/edit`)
      } else {
  ...
  const {
    ...
    mutationFn: async () => {
      if (!proposalId) throw new Error('Save the draft first before submitting for review.')
  ...
  return (
    <div style={pageStyle}>
      ...
      >
        ...
        >
          Once submitted, you cannot edit the proposal until the review is complete. Are you sure
          you want to submit?
        </p>
        {submitError && (
    ...
    <div>
      ...
      <div style={fieldGroupStyle}>
        ...
        <textarea
          ...
          placeholder="Brief one-paragraph summary of your proposal"
          rows={3}
        />
      </div>
      <div style={fieldGroupStyle}>
...
function StepReview({
  ...
  return (
    <div>
      ...
      >
        Review your proposal details before submitting for review.
      </p>
      ...
      >
        ...
        <button
          ...
          style={{
            ...primaryButtonStyle,
            opacity: proposalId ? 1 : 0.5,
            cursor: proposalId ? 'pointer' : 'not-allowed',
          }}
          onClick={proposalId ? onOpenDialog : undefined}
          disabled={!proposalId}
