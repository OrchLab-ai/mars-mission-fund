...
export function FundingProgressSection({ proposal, className }: FundingProgressSectionProps) {
  const percentage = Math.min(100, (proposal.raisedAmount / proposal.goalAmount) * 100)
  const isComplete = percentage >= 100

  return (
    <div style={sectionStyle} className={className}>
      <div style={amountsStyle}>
        <span style={raisedStyle}>{formatCurrency(proposal.raisedAmount)}</span>
        <span style={targetStyle}>raised of {formatCurrency(proposal.goalAmount)} goal</span>
      </div>
      ...
      <div style={statsGridStyle}>
        <div style={statItemStyle}>
          <span style={statValueStyle}>{proposal.contributorCount.toLocaleString('en-US')}</span>
          <span style={statLabelStyle}>Contributors</span>
        </div>
        <div style={statItemStyle}>
          <span style={statValueStyle}>{getTimeRemaining(proposal.deadline)}</span>
          <span style={statLabelStyle}>Time Left</span>
        </div>
      </div>

      <Button variant="primary" href={'/contribute/' + proposal.id}>
        Contribute Now
      </Button>
    </div>
  )
