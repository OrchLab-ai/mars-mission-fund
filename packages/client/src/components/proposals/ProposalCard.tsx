...
export function ProposalCard({ proposal }: ProposalCardProps) {
  ...
  const fundingPct = Math.min(100, (proposal.raisedAmount / proposal.goalAmount) * 100)
  const timeRemaining = getTimeRemaining(proposal.deadline)

  return (
    <Card accent>
      <Badge variant="active">{proposal.category}</Badge>
      <h3 style={titleStyle}>{proposal.title}</h3>
      <p style={summaryStyle}>{excerpt}</p>
      <ProgressBar
        value={fundingPct}
        complete={fundingPct >= 100}
        label={`${proposal.title} funding progress: ${Math.round(fundingPct)}% funded`}
      />
