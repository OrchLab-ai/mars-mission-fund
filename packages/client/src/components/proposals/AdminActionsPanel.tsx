...
import { verifyMilestone, returnMilestone, approveCancel } from '../../api/proposals'
import type { ProposalDetail, Milestone } from '../../api/proposals'
...
function MilestoneVerifyItem({
  ...
  const verifyMutation = useMutation({
    mutationFn: () => verifyMilestone(proposalId, milestone.id),
    onSuccess: () => {
  ...
  const returnMutation = useMutation({
    mutationFn: () => returnMilestone(proposalId, milestone.id, feedback),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['proposal', proposalId] })
    },
    onError: () => {
...
function MilestoneVerificationSection({ proposal }: { proposal: ProposalDetail }) {
  const submittedMilestones = proposal.milestones.filter((m) => m.status === 'Submitted')
  ...
  return (
    <div style={sectionStyle}>
      ...
      {submittedMilestones.map((milestone) => (
        <MilestoneVerifyItem key={milestone.id} proposalId={proposal.id} milestone={milestone} />
      ))}
    </div>
  )
}

function CancellationApprovalSection({ proposal }: { proposal: ProposalDetail }) {
  const queryClient = useQueryClient()
  const [error, setError] = useState<string | null>(null)

  const approveMutation = useMutation({
    mutationFn: () => approveCancel(proposal.id),
    onSuccess: () => {
  ...
  return (
    <div style={sectionStyle}>
      ...
      <p style={metaStyle}>
        Requested:{' '}
        {proposal.cancellationRequestedAt
          ? new Date(proposal.cancellationRequestedAt).toLocaleDateString()
          : ''}
      </p>
      {error && (
        <span role="alert" style={errorStyle}>
...
export function AdminActionsPanel({ proposal, user }: AdminActionsPanelProps) {
  const isAdmin = user?.role === 'Administrator' || user?.role === 'SuperAdministrator'

  if (!isAdmin) return null

  const showMilestoneVerification = proposal.status === 'Settlement'
  const showCancellationApproval =
    proposal.status === 'Live' && proposal.cancellationRequestedAt != null
  ...
  return (
    <div style={panelStyle} aria-label="Admin actions">
      ...
      {showMilestoneVerification && <MilestoneVerificationSection proposal={proposal} />}
      {showCancellationApproval && <CancellationApprovalSection proposal={proposal} />}
    </div>
  )
}
