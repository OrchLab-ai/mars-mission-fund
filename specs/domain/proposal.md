...
> **Local demo scope**: The proposal state machine, submission flow, review pipeline, and milestone verification workflow are **real** — they are the core demo feature. Appeal processes, deadline enforcement automation, and stretch goal mechanics are theatre for the workshop. The local demo focuses on the happy path: Draft → Submitted → Under Review → Approved → Live → Funded → Settlement → Complete.

This spec governs the full proposal lifecycle on Mars Mission Fund — from initial project submission through review, approval, live fundraising, milestone verification, and final settlement or failure handling.

**What this spec covers**:

- Proposal state machine and all valid transitions.
...
- Live proposal management: public page, funding progress, stretch goals, deadline enforcement.
- Milestone definition, evidence submission, verification, and staged fund release.
- Proposal completion and settlement workflow.
- Proposal failure, deadline expiry, and refund triggers.
...
- **Section 1.2 — Data Access**: All proposal data queries use parameterised queries through the data access layer.
- **Section 1.4 — Input Validation**: All proposal submission fields are validated and sanitised at the system boundary.
- **Section 1.5 — Authentication & Authorisation**: Every proposal endpoint is authenticated and authorised. Role-based access controls enforce who can submit, review, approve, and manage proposals.
- **Section 1.7 — Logging & Auditability**: Every proposal state mutation is logged with timestamp, actor identity, action, and affected resource.
- **Section 3.4 — Code Review & Approval**: Changes to proposal flows affecting security surfaces require a second security reviewer.
- **Section 4.2 — Test Coverage**: Proposal business logic requires 90% unit test coverage. Proposal API endpoints require 100% integration test coverage of documented contracts.
- **Section 5 — API & Interface Contracts**: Proposal APIs are versioned, backward-compatible within a version, and use the standard error response format.

### From [Architecture](L3-001)

- Proposal service boundaries, data model, and inter-service communication patterns as defined in the system architecture.

### From [Audit](L3-006)

- All proposal state transitions, review decisions, milestone verifications, and fund release triggers are audit-logged per the immutable event stream architecture defined in L3-006.
- Audit entries for proposal actions include the proposal ID, actor, previous state, new state, and rationale (where applicable).
...
| **Live**         | The proposal is publicly visible and accepting contributions.                                                                     |
| **Funded**       | The proposal has reached its minimum funding target. Contributions continue until the deadline or maximum funding cap is reached. |
| **Suspended**    | Proposal temporarily frozen (e.g., creator KYC revoked). No new contributions accepted. Awaiting resolution.                      |
| **Failed**       | The proposal deadline has passed without reaching the minimum funding target.                                                     |
| **Settlement**   | Funds are being disbursed to the creator per the milestone plan.                                                                  |
| **Complete**     | All milestones verified and all funds disbursed (or proposal otherwise finalised).                                                |
| **Cancelled**    | Proposal cancelled by the creator or an administrator before completion.                                                          |
...
| Submitted    | Under Review | Reviewer          | Reviewer claims proposal from review queue (FIFO)                                                                      |
...
| Live         | Funded       | System            | Minimum funding target reached; proposal remains live for additional contributions until deadline or maximum cap       |
...
| Suspended    | Live         | Admin             | Issue resolved (e.g., KYC re-verified); proposal resumes accepting contributions                                       |
| Suspended    | Funded       | Admin             | Issue resolved; proposal was already funded before suspension                                                          |
...
- Every state transition is audit-logged per [Audit](L3-006) with: proposal ID, previous state, new state, actor, timestamp, and rationale.
- No transition may skip states (e.g., Draft cannot go directly to Live).
- Only one active transition may be in progress at a time per proposal.
- The Cancelled state is terminal — no transitions out.
- The Complete state is terminal — no transitions out.
- When the maximum funding cap is reached, the proposal stops accepting new contributions. The proposal remains in Funded state until the deadline.
...
| **Funding**            | Minimum funding target (USD), maximum funding cap (USD), funding deadline, budget breakdown, proposal category | Budget breakdown must account for 100% of the minimum target. The platform uses USD. Maximum cap is the point at which the proposal stops accepting contributions. |
...
Every proposal must be assigned exactly one primary category from the platform taxonomy.
...
- When a proposal enters Submitted state, it is added to a shared review queue ordered by submission time (FIFO).
- A reviewer claims the next available proposal from the queue. No automatic assignment — reviewers pull work when ready.
- A reviewer may recuse themselves, returning the proposal to the queue.
- Admin users may manually reassign a proposal to a different reviewer.
...
- **Request Clarification** — sends questions back to the creator without changing state (proposal remains Under Review).

### 5.4 Review SLA

- Target time from Submitted to review outcome: 5 business days.
- At 3 business days without a reviewer claiming the proposal, an alert is sent to all reviewers.
- At 5 business days, the proposal is escalated to Admin for manual assignment or action.
...
- Approved proposals are cleared to go live.
...
> **Local demo scope**: The appeal process is **not implemented** in the demo. Rejected proposals
...
## 7. Live Proposal Management

### 7.1 Public Proposal Page

- Approved proposals that have launched are publicly visible.
- The proposal page displays: all submission fields (formatted per [Brand](L2-001)), funding progress (current amount vs. target), contributor count, time remaining, milestone plan, and stretch goals (if any).
- The proposal page provides a contribution call-to-action that initiates the donor contribution flow (reference [Donor](L4-003) and [Payments](L4-004)).
...
- If stretch goals are defined, they become visible on the proposal page once the minimum funding target is reached.
...
- Every live proposal has a deadline.
- The system automatically transitions the proposal at deadline:
  - If minimum funding target met → **Funded** state.
  - If minimum funding target not met → **Failed** state.

#### Deadline Extensions

> **Local demo scope**: Deadline extension requests are **not implemented** in the demo. Proposals
...
- Creators may request a deadline extension while the proposal is Live.
- Extensions require Admin approval.
- Maximum single extension: 30 days.
- Maximum total extensions per proposal: 90 days (cumulative).
...
**AC-CAMP-019**: Given a live proposal, when a creator requests a deadline extension with written justification, and an Admin approves it, then the deadline is extended, all contributors are notified, and the extension is audit-logged.

**AC-CAMP-020**: Given a live proposal, when a creator requests a deadline extension that would exceed the 90-day cumulative limit, then the extension request is rejected with an explanation.

### 7.5 Proposal Updates

- Creators may post updates to their live proposal (text and media).
- Updates are visible on the proposal page and trigger notifications to contributors.
...
**AC-CAMP-009**: Given a proposal is in "Approved" state, when the creator launches it, then the proposal transitions to "Live" and is publicly visible on the platform.

**AC-CAMP-010**: Given a live proposal, when a contribution is confirmed by the payments system, then the funding progress is updated in near-real-time on the proposal page.

**AC-CAMP-011**: Given a live proposal, when the minimum funding target is reached, then the proposal transitions to "Funded" state and remains open for contributions until the deadline or maximum funding cap is reached.

**AC-CAMP-012**: Given a live proposal, when the deadline is reached and the minimum funding target has not been met, then the proposal transitions to "Failed" state and refunds are triggered via the payments system.

**AC-CAMP-021**: Given a live or funded proposal, when a creator requests a milestone change, and an Admin approves it, then the milestone plan is updated, all contributors are notified with a summary of changes, and the original plan is preserved in the audit trail.

**AC-CAMP-022**: Given a funded proposal, when a contribution would exceed the maximum funding cap, then the contribution is rejected and the donor is informed the proposal has reached its cap.
...
**AC-CAMP-013**: Given a proposal in "Settlement" state, when a creator submits evidence for a milestone, then the evidence is recorded and an Admin is notified for verification.
...
The system transitions the proposal to **Complete** state.

### 9.2 Settlement Workflow

1. Proposal reaches deadline in Funded state → transitions to Settlement.
...
1. After final milestone verification and disbursement, proposal transitions to Complete.

**AC-CAMP-016**: Given all milestones are verified and all funds disbursed, when the final disbursement is confirmed, then the proposal transitions to "Complete" state and the creator is notified.
...
A proposal fails when:

- The deadline is reached and the minimum funding target has not been met.

The funding model is **flexible with a minimum threshold**: proposals that meet their minimum target keep what they raise (up to the maximum cap). Proposals that do not reach the minimum target are failed and all contributions are refunded.

### 10.2 Failure Handling

1. Proposal transitions to **Failed** state.
1. System triggers a refund for all contributions via [Payments](L4-004).
1. Creator is notified that the proposal has failed.
1. Contributors are notified that their contributions will be refunded.
1. Failed proposals remain visible on the platform (marked as failed) for transparency.

### 10.3 Cancellation

- A creator may cancel a Live proposal. If contributions exist, the Admin must approve the cancellation.
...
**AC-CAMP-017**: Given a live proposal, when the deadline passes without the minimum funding target being met, then the proposal transitions to "Failed" and refunds are triggered for all contributions.

**AC-CAMP-018**: Given a creator requests cancellation of a live proposal with existing contributions, when an Admin approves the cancellation, then the proposal transitions to "Cancelled" and all contributions are refunded.

---

## 11. Interface Contracts

### 11.1 Proposal ↔ Payments ([Payments](L4-004))

| Event                       | Proposal Publishes                                                                  | Payments Responds                                                           |
| --------------------------- | ----------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| Proposal goes Live          | `ProposalLive` — proposal ID, minimum funding target, maximum funding cap, deadline | Payments creates escrow account for proposal                                |
| Contribution received       | —                                                                                   | Payments notifies proposal of confirmed contribution amount                 |
| Funding target reached      | `FundingTargetReached` — proposal ID                                                | Payments acknowledges; no action until settlement                           |
| Proposal funded at deadline | `ProposalFunded` — proposal ID, total collected                                     | Payments holds funds in escrow pending milestone verification               |
| Milestone verified          | `MilestoneVerified` — proposal ID, milestone ID, disbursement amount                | Payments executes disbursement to creator                                   |
| Proposal failed             | `ProposalFailed` — proposal ID                                                      | Payments executes refunds to all contributors                               |
| Proposal suspended          | `ProposalSuspended` — proposal ID, reason                                           | Payments stops accepting new contributions for this proposal                |
| Proposal resumed            | `ProposalResumed` — proposal ID                                                     | Payments resumes accepting contributions (if below cap and before deadline) |
| Proposal cancelled          | `ProposalCancelled` — proposal ID                                                   | Payments executes refunds for any existing contributions                    |
| All milestones complete     | `ProposalComplete` — proposal ID                                                    | Payments confirms all funds settled; closes escrow                          |

Both specs must define these events.
The event schema and transport mechanism are defined in [Architecture](L3-001).

### 11.2 Proposal ↔ Donor ([Donor](L4-003))

| Direction        | Data Provided                                                                                                                          |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| Proposal → Donor | Proposal metadata for discovery: title, summary, category, funding progress, deadline, status, hero image, Mars-mission alignment tags |
| Donor → Proposal | Contribution data: contributor count, total contributed (aggregated), individual contribution events (via [Payments](L4-004))          |

- The proposal service provides read-only proposal data to the donor discovery and search systems.
- Contribution data flows through [Payments](L4-004), not directly from donor to proposal.

### 11.3 Proposal ↔ Account ([Account](L4-001))

| Role               | Proposal Permissions                                                                                                                                                                               |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Creator**        | Create drafts, edit drafts, submit proposals, launch approved proposals, post updates, submit milestone evidence, request cancellation, request deadline extensions, request milestone changes     |
...
| **Admin**          | Reassign reviewers, approve cancellations, verify milestones, handle appeals, approve/reject deadline extensions, approve/reject milestone changes, manage suspended proposals (restore or cancel) |
| **Backer** (Donor) | View live proposals, contribute (via [Payments](L4-004)), view updates and milestone progress                                                                                                      |

Role definitions and assignment are governed by [Account](L4-001).
Proposal enforces permissions based on the authenticated user's role.

### 11.4 Proposal ↔ KYC ([KYC](L4-005))

- Proposal submission requires the creator to have a verified KYC status.
- Proposal checks KYC verification status at submission time via the KYC domain interface.
- If a creator's KYC status is revoked while a proposal is **Live**: the proposal is suspended (no new contributions accepted), the creator and Admin are notified, and the Admin determines next steps (cancel with refunds, or allow the creator to re-verify within 14 days). If re-verification is not completed within 14 days, the proposal is cancelled and all contributions are refunded via [Payments](L4-004).
...
**AC-CAMP-023**: Given a live proposal, when the creator's KYC status is revoked, then the proposal transitions to "Suspended", no new contributions are accepted, and the creator and Admin are notified.

**AC-CAMP-024**: Given a suspended proposal, when the creator completes KYC re-verification within 14 days, then the Admin may restore the proposal to its previous state (Live or Funded).

**AC-CAMP-025**: Given a suspended proposal, when 14 days elapse without KYC re-verification, then the proposal is cancelled and all contributions are refunded via the payments system.

---

## 12. Glossary

Domain vocabulary introduced during the Proposal Lifecycle milestone.

| Term | Definition |
| ---- | ---------- |
| **Review Queue** | FIFO set of Submitted proposals available for Reviewers to claim. Reviewers self-assign (pull model) — proposals are not assigned by admins unless escalated past the 5-day SLA. |
| **Settlement** | Post-deadline fund disbursement workflow. Triggered when a Funded proposal passes its deadline. Admin verifies each milestone and releases escrowed funds in staged tranches per the milestone plan. |
| **Stretch Goal** | An optional funding tier above the minimum target that unlocks additional deliverables when crossed. Stretch goals are visible on the proposal page once the minimum target is reached and activate automatically as each threshold is met. |
...
| **Deadline Enforcement** | Automatic system transition from Live to Funded (minimum met) or Failed (minimum not met) when the proposal deadline passes. Triggered lazily on next contribution attempt or explicitly by a scheduled check. |
...
| ---------- | ------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| March 2026 | 0.1     | —      | Initial stub. Proposal state machine, submission flow, review pipeline, approval/rejection, live management, milestones, completion, failure, interface contracts with Payments, Donor, Account, and KYC.                                                                                                                                                                                                                                                                |
| March 2026 | 0.2     | —      | Resolved all open questions. Funding: USD, min $1M, max $1B. Flexible funding model (min threshold + max cap). Duration: 1 week–1 year. Pull-based reviewer queue (FIFO), 5-day SLA. Deadline extensions (30-day max, 90-day cumulative). Milestone change requests with Admin approval and contributor notification. KYC revocation handling (14-day re-verification window). Unlimited simultaneous proposals. 10-category taxonomy based on NASA Technology Taxonomy. |
